...
import {
    ...
    Image as ImageIcon, // Added for About Me Photo Placeholder
    Download, // <-- ADDED FOR RESUME
    ArrowLeft, // <-- ADDED FOR MOBILE CONTROLS
    ArrowRight, // <-- ADDED FOR MOBILE CONTROLS
    ArrowUp // <-- ADDED FOR MOBILE CONTROLS
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { v4 as uuidv4 } from 'uuid'; // Make sure uuid is installed: npm install uuid
...
export default function PortfolioCarGame() {
    ...
    // --- Game State ---
    const [position, setPosition] = useState({ x: 50, y: SPRITE_LOGICAL_Y });
    const [speed, setSpeed] = useState({ x: 0, y: 0 });
    const [backgroundPosition, setBackgroundPosition] = useState(0);
    const [isDayTime, setIsDayTime] = useState(true);
    const [currentSectionKey, setCurrentSectionKey] = useState('intro');
    ...
    // --- Portfolio Data --- (Remains the same as provided)
    const sections = {
        'intro': {
            title: 'Welcome!', icon: <PlayCircle size={20} className="text-teal-300" />,
            contentTitle: "Sunny Anand", contentSubtitle: "Interactive Portfolio", contentAction: "Run → to Explore", // Changed 'Drive' to 'Run'
    ...
    const sectionKeys = Object.keys(sections);
    const numSections = sectionKeys.length;
    const sectionsToWarnAbout = ['skills', 'projects', 'certifications', 'education', 'resume', 'contact'];
    ...
    useEffect(() => {
        const checkForMobile = () => {
            // Simple check for touch events or common mobile user agents
            const isTouchDevice = ('ontouchstart' in window) || (navigator.maxTouchPoints > 0);
            const isMobileUserAgent = /Mobi|Android/i.test(navigator.userAgent);
            setIsMobile(isTouchDevice || isMobileUserAgent);
        };
        checkForMobile();
        // Optional: Re-check on resize if needed, though initial check is usually sufficient
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            ...
            // Allow 'c' key to toggle controls regardless of mobile
            if (key === 'c') {
        ...
        return () => {
            ...
            keysPressedRef.current = {}; // Clear keys on cleanup
        };
    }, []); // Empty dependency array means this runs once on mount and cleans up on unmount

    // --- Touch Input Handling --- (NEW)
    const handleTouchStart = useCallback((key) => (e) => {
        e.preventDefault(); // Prevent screen scrolling/zooming on touch
        keysPressedRef.current[key] = true;
    }, []);
    ...
    const createParticles = useCallback((count, type, options = {}) => {
        ...
        setParticles(prev => [...prev.slice(-200), ...newParticles]);
    }, []); // Add positionRef and speedRef if needed, but likely okay as read-only access
    ...
    // --- Main Game Loop --- (No changes needed here, physics uses keysPressedRef)
    const gameLoop = useCallback((timestamp) => {
        const now = timestamp || performance.now(); const deltaTime = (now - lastTimeRef.current) / 1000; lastTimeRef.current = now; const dt = Math.min(deltaTime, 0.05);
        let currentSpeed = { ...speedRef.current }; let currentPosition = { ...positionRef.current }; let currentBackgroundPos = backgroundPositionRef.current;
        const keys = keysPressedRef.current; // Read directly from the ref

        // --- Movement Physics ---
        // Keys map: 'arrowright', 'd', 'arrowleft', 'a', 'arrowup', ' '
        const isMovingRight = keys['arrowright'] || keys['d'];
        const isMovingLeft = keys['arrowleft'] || keys['a'];

        if (isMovingRight) { currentSpeed.x = Math.min(currentSpeed.x + ACCELERATION * dt, MAX_SPEED); } // Multiply by dt
        else if (isMovingLeft) { currentSpeed.x = Math.max(currentSpeed.x - ACCELERATION * dt, -MAX_SPEED); } // Multiply by dt
        else { currentSpeed.x *= (DECELERATION_FACTOR ** (60 * dt)); if (Math.abs(currentSpeed.x) < 10) currentSpeed.x = 0; } // Apply deceleration over time

        const currentlyOnGround = currentPosition.y >= SPRITE_LOGICAL_Y;
        setIsOnGround(currentlyOnGround);
        const isTryingToJump = keys['arrowup'] || keys[' ']; // Space or ArrowUp

        if (!currentlyOnGround || currentSpeed.y < 0) {
            currentSpeed.y += GRAVITY * dt;
        } else {
            currentSpeed.y = 0; // Ensure vertical speed is 0 when on ground
        }

        if (isTryingToJump && currentlyOnGround) {
            currentSpeed.y = JUMP_VELOCITY;
        }

        // --- Calculate Next Position ---
        let nextX = currentPosition.x + currentSpeed.x * dt;
        let nextY = currentPosition.y + currentSpeed.y * dt;

        // --- Ground Collision ---
        if (nextY > SPRITE_LOGICAL_Y) {
             nextY = SPRITE_LOGICAL_Y;
             // Don't reset speed here, allow gravity to handle it if jump happened near ground
             if (currentSpeed.y > 0) currentSpeed.y = 0; // Reset only if moving downwards into ground
        }

         // --- Simple Coin Generation ---
        if (Math.abs(currentSpeed.x) > 10 && Math.random() < 0.01) { setCoinCount(prev => prev + 1); createParticles(2, 'coin', { pos: { x: positionRef.current.x + SPRITE_WIDTH / 2, y: positionRef.current.y - 10 } }); }

        // --- Screen Boundaries & Background Scrolling ---
        if (nextX < LEFT_BOUNDARY) { nextX = LEFT_BOUNDARY; currentSpeed.x = 0; }
        else if (nextX > SCROLL_THRESHOLD && currentSpeed.x > 0) { const scrollAmount = (nextX - SCROLL_THRESHOLD); currentBackgroundPos -= scrollAmount; nextX = SCROLL_THRESHOLD; }
        else if (nextX < SCROLL_THRESHOLD && currentSpeed.x < 0 && currentBackgroundPos < 0) { const scrollAmount = currentSpeed.x * dt; const maxScrollBack = Math.abs(scrollAmount); currentBackgroundPos = Math.min(0, currentBackgroundPos - scrollAmount); nextX = Math.max(LEFT_BOUNDARY, nextX); } // Prevent scrolling past start
        ...
        if (sectionData?.type === 'list' && sectionData.items) {
            ...
            // Reset max count when leaving a list section
             if (maxVisibleIndexPerSectionRef.current[currentSectionKeyRef.current] !== undefined && sectionData?.type !== 'list') {
                 maxVisibleIndexPerSectionRef.current[currentSectionKeyRef.current] = 0;
             }
             newVisibleCount = 0; // Reset visible count for non-list sections
        }
        setVisibleItemCount(newVisibleCount);
        ...
        if (newSectionKey !== currentSectionKeyRef.current) {
             setCurrentSectionKey(newSectionKey);
             // No need to update ref here, useEffect hook handles it
             // Clear max visible count for the *new* section if it's not a list type upon entering
             if (!sections[newSectionKey]?.items || sections[newSectionKey]?.type !== 'list') {
                 maxVisibleIndexPerSectionRef.current[newSectionKey] = 0;
             }
        }
    ...
    }, [ // Keep dependencies minimal for the core loop
        updateParticles, createParticles, numSections, sectionsToWarnAbout, // Callbacks & constants
        showSectionWarning, approachingSectionKey, // State used directly in calculations
        // Implicit dependencies: Constants like SECTION_WIDTH, SPRITE_LOGICAL_Y etc.
        // Refs are accessed directly so not needed here. State setters are stable.
    ]);
    ...
    }, [gameLoop]); // Re-run if gameLoop reference changes (it shouldn't if dependencies are correct)
    ...
    useEffect(() => {
        ...
        if (isProjectsSection && projectItems?.length > 0) {
            setProjectImageIndex(0); // Reset to first image when entering section
            projectSlideshowIntervalRef.current = setInterval(() => {
                setProjectImageIndex(prevIndex => (prevIndex + 1) % projectItems.length);
            }, PROJECT_SLIDESHOW_INTERVAL);
        }
    ...
    }, [currentSectionKey]); // Only depends on currentSectionKey


    // --- Styles & Render Prep --- (Remain the same)
    const getSkyColor = () => { return isDayTime ? 'bg-gradient-to-b from-sky-400 via-sky-300 to-sky-100' : 'bg-gradient-to-b from-slate-900 via-indigo-900 to-purple-900'; };
    const getGroundColor = () => { return isDayTime ? 'bg-gradient-to-b from-lime-600 to-green-800' : 'bg-gradient-to-b from-emerald-900 to-green-950'; };
    // const showIdleAnimation = isOnGround && Math.abs(speed.x) < 5; // Keep if needed
    const currentSectionData = sections[currentSectionKey];

    // --- Animation Variants --- (Remain the same)
    const aboutOverlayVariants = { hidden: { opacity: 0, scale: 0.9 }, visible: { opacity: 1, scale: 1, transition: { duration: 0.5, ease: "easeOut", staggerChildren: 0.1 } }, exit: { opacity: 0, scale: 0.95, transition: { duration: 0.3, ease: "easeIn" } } };
    const aboutImageVariants = { hidden: { opacity: 0, y: -20, scale: 0.8 }, visible: { opacity: 1, y: 0, scale: 1, transition: { type: "spring", stiffness: 120, damping: 10 } } };
    const aboutTextVariants = { hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.4, ease: "easeOut" } }, exit: { opacity: 0, y: -10, transition: { duration: 0.2, ease: "easeIn" } } };
    const welcomeOverlayVariants = { hidden: { opacity: 0 }, visible: { opacity: 1, transition: { duration: 0.4, ease: "easeOut", staggerChildren: 0.2, delayChildren: 0.1 } }, exit: { opacity: 0, transition: { duration: 0.3, ease: "easeIn" } } };
    const welcomeTextVariants = { hidden: { opacity: 0, y: 30, scale: 0.95 }, visible: { opacity: 1, y: 0, scale: 1, transition: { type: "spring", stiffness: 100, damping: 12 } }, exit: { opacity: 0, y: -15, scale: 0.98 } };
    const welcomeActionVariants = { hidden: { opacity: 0, y: 15 }, visible: { opacity: 1, y: 0, transition: { delay: 0.5, type: "spring", stiffness: 80 } }, exit: { opacity: 0 } };
    const projectImageVariants = { hidden: { opacity: 0, scale: 1.05, transition: { duration: 0.5, ease: 'easeOut' } }, visible: { opacity: 1, scale: 1, transition: { duration: 0.8, ease: 'easeIn' } }, exit: { opacity: 0, scale: 0.98, transition: { duration: 0.5, ease: 'easeOut' } } };
    const warningVariants = { hidden: { opacity: 0, scale: 0.5, y: 50, rotate: -5 }, visible: { opacity: 1, scale: 1, y: 0, rotate: 0, transition: { type: 'spring', stiffness: 150, damping: 15, duration: 0.4 } }, exit: { opacity: 0, scale: 0.8, y: -30, transition: { duration: 0.2, ease: 'easeIn' } } };
    const contactItemVariants = { hidden: { opacity: 0, x: -20 }, visible: { opacity: 1, x: 0 } };
    const sectionBoxVariants = { hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.3, ease: "easeOut" } } };
    ...
    return (
        <div className="w-full h-screen overflow-hidden relative bg-black focus:outline-none select-none" tabIndex={0}>
            ...
            {/* --- Foreground Elements (Z: 35-50) --- (Dinosaur replaces Car) */}
             ...
             {/* --- Player Sprite (Dinosaur) --- (Z: 50) */}
            <div
                className="absolute z-50 transition-transform duration-75 ease-out pointer-events-none" // Added pointer-events-none
                style={{
                    left: position.x,
                    top: position.y - SPRITE_VISUAL_HEIGHT,
                    width: `${SPRITE_WIDTH}px`,
                    height: `${SPRITE_VISUAL_HEIGHT}px`,
                    transform: `rotate(${Math.max(-15, Math.min(15, speed.y / 25))}deg)`,
                    //transformScaleX: speed.x < -1 ? -1 : 1, // Flip handled by img below
                }}
            >
                <img
                    src="/running.gif" // <-- Ensure this path is correct
                    alt="Player Dinosaur"
                    className="w-full h-full object-contain"
                    style={{ transform: speed.x < -1 ? 'scaleX(-1)' : 'scaleX(1)' }} // Flip based on horizontal speed
                />
            </div>
            ...
            <AnimatePresence>
                ...
                {currentSectionKey === 'about' && currentSectionData && (
                    <motion.div key="about-me-overlay" variants={aboutOverlayVariants} initial="hidden" animate="visible" exit="exit" className="absolute inset-0 z-55 flex items-center justify-center pointer-events-none">
                        ...
                        <motion.div className="relative max-w-3xl w-full mx-auto p-6 md:p-8 text-center flex flex-col items-center">
                             <motion.div variants={aboutImageVariants} className="mb-5 md:mb-6">
                                {currentSectionData.profilePictureUrl ? (
                                    <img
                                        src={currentSectionData.profilePictureUrl}
                                        alt="Aadarsh Raj"
                                        className="w-32 h-32 md:w-40 md:h-40 rounded-full object-cover border-4 border-sky-300 shadow-xl"
                                    />
                                ) : (
                 ...
                 <AnimatePresence>
                    {showSectionWarning && approachingSectionKey && sections[approachingSectionKey] && (
                        <motion.div
                            ...
                            animate="visible"
                            exit="exit"
                            className="bg-black/75 backdrop-blur-md rounded-xl p-6 md:p-8 shadow-2xl border-2 border-yellow-500/80 text-center max-w-md mx-auto" // Added max-width
                        >
                            <div className="flex items-center justify-center gap-3 md:gap-4 mb-2">
                                <AlertTriangle size={32} className="text-yellow-400 animate-pulse flex-shrink-0" />
                                <h3 className="text-2xl md:text-4xl font-bold text-white text-shadow-md whitespace-nowrap">
            ...
            {/* --- UI Layer (z-60) --- (Section Title, Items, Contact, Resume, Mobile Controls) */}
             {/* Section Title Banner (Conditionally Rendered) */}
             {currentSectionKey !== 'intro' &&
              currentSectionKey !== 'about' &&
              currentSectionKey !== 'projects' &&
            ...
            {currentSectionData?.type === 'list' && (
             ...
             {/* Top Right UI (Coins, Controls Toggle Info) */}
            <div className="absolute top-5 right-5 flex flex-col items-end gap-2 z-60">
                <div className="flex items-center gap-2 bg-black/70 backdrop-blur-sm p-2 px-4 rounded-lg text-white shadow-xl border border-white/10"> <Diamond size={18} className="text-yellow-400 filter drop-shadow animate-pulse" /> <span className="text-lg font-bold text-shadow-sm">{coinCount}</span> </div>
                 {/* Show controls toggle info only on non-mobile */}
                 {!isMobile && (
                     <AnimatePresence> {showControls && ( <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="bg-black/70 backdrop-blur-sm p-3 rounded-lg text-white text-xs shadow-xl border border-white/10 leading-relaxed w-max"> <h3 className="font-bold mb-1 uppercase text-shadow-sm text-center">Controls</h3> <p>← → / A D: Move</p> <p>↑ / Space: Jump</p> <p>C: Toggle</p> </motion.div> )} </AnimatePresence>
                 )}
            </div>
             ...
             {currentSectionData?.type === 'resumeDownload' && (
                 <motion.div
                    ...
                    initial="hidden"
                    animate="visible"
                    className="absolute bottom-5 left-5 max-w-xs w-full bg-gradient-to-br from-gray-900/80 via-black/70 to-gray-800/80 backdrop-blur-md p-4 rounded-xl shadow-2xl z-60 border border-green-400/30 text-white"
                 >
                     <h3 className="text-xl font-bold mb-4 flex items-center gap-2 text-shadow-sm">
                         {currentSectionData.icon || <Download size={20}/>}
                         {currentSectionData.title || "Download Resume"}
                     ...
                     <div className="text-center">
                         <a
                             href={currentSectionData.resumeUrl || '#'}
                             download="Resume.pdf"
                             target="_blank"
                             rel="noopener noreferrer"
                             className="inline-flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 rounded-md text-white font-semibold transition-colors duration-200 shadow-md active:bg-green-800" // Added active state
                             // Prevent touch event propagation if needed, though not strictly necessary for 'a' tag
                             // onTouchStart={(e) => e.stopPropagation()}
                         >
                             <Download size={16} />
                             {currentSectionData.linkText || "Download Now"}
                         </a>
                         {(!currentSectionData.resumeUrl || currentSectionData.resumeUrl === '/Resume.pdf')}
                     </div>
                 </motion.div>
             )}
            ...
            {isMobile && (
                <div
                    className="fixed bottom-4 left-4 right-4 flex justify-between items-end z-70 pointer-events-none select-none"
                    aria-hidden="true" // Hide from screen readers as they are visual controls
                >
                    {/* Left/Right Buttons */}
                    <div className="flex gap-3 pointer-events-auto">
                        <button
                            ...
                            onTouchCancel={handleTouchEnd('arrowleft')} // Handle touch cancel
                            className="bg-black/40 text-white p-4 rounded-full active:bg-black/60 transition-colors shadow-lg backdrop-blur-sm border border-white/10"
                            aria-label="Move Left" // Keep aria-label for accessibility checkers, though parent is hidden
                        >
                            <ArrowLeft size={28} /> {/* Slightly larger icons for touch */}
                        </button>
                        <button
                            onTouchStart={handleTouchStart('arrowright')}
                            onTouchEnd={handleTouchEnd('arrowright')}
                            onTouchCancel={handleTouchEnd('arrowright')} // Handle touch cancel
                            className="bg-black/40 text-white p-4 rounded-full active:bg-black/60 transition-colors shadow-lg backdrop-blur-sm border border-white/10"
                    ...
                    <div className="pointer-events-auto">
                         <button
                            onTouchStart={handleTouchStart(' ')} // Use space key for jump
                            onTouchEnd={handleTouchEnd(' ')}
                            onTouchCancel={handleTouchEnd(' ')} // Handle touch cancel
                            className="bg-black/40 text-white p-5 rounded-full active:bg-black/60 transition-colors shadow-lg backdrop-blur-sm border border-white/10"
                            aria-label="Jump"
                         >
                            <ArrowUp size={32} /> {/* Slightly larger jump icon */}
                        </button>
                    </div>
                </div>
            )}
